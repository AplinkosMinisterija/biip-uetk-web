...
import { availableMimeTypes, handleHasCoordinatesField, isMapEditAttribute } from './functions';
...
export const validateForm = Yup.object().shape({
  ...
  geom: Yup.object().when(['type', 'editFields'], {
    is: (type: FormType, editFields: { attribute: FormDataFields; value: any }[]) => {
  ...
  editFields: Yup.array().when(['type'], {
    ...
    then: Yup.array().of(
      Yup.object().shape({
        attribute: Yup.string().required(validationTexts.requireSelect).nullable(),
  ...
  data: Yup.lazy((_, ctx) => {
    ...
    const requirePondedRiver = [FormObjectType.POND, FormObjectType.PONDED_LAKE].includes(
      objectType,
    );
    ...
    const requirePowerAndMaxPressureHeight = isEqual(objectType, FormObjectType.HYDRO_POWER_PLANT);
    ...
    if (requirePowerAndMaxPressureHeight) {
      obj.maxPressureHeight = Yup.string().required(validationTexts.requireText);
